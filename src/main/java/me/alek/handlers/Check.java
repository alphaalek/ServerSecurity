...
public class Check {
    ...
    public static CheckResult check(CheckAdapter handler, File file, Path rootFolder, CacheContainer cache) {
        ...
        if (handler instanceof ForceOPCheck) {
            AcceptedPluginsForceOPContainer container = Scanner.getAcceptedPluginsForceOPContainer();
            if (container.check(file.getName())) return null;
        }
